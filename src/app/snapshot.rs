...
pub struct AppSnapshot {
    ...
    pub listening: bool,
    pub who_is: u64,
    pub read_property: u64,
    pub read_property_multiple: u64,
...
pub fn device_points(sim: &Simulation, device_id: u32) -> Vec<PointRow> {
    ...
    device
        ...
        .map(|p| PointRow {
            label: p.label.clone(),
            object_type: format!("{:?}", p.object_type),
...
pub async fn build_snapshot(
    ...
    let last_client = metrics.last_client().map(|(addr, _)| addr.to_string());
    ...
    AppSnapshot {
        ...
        who_is: metrics.who_is_count(),
        read_property: metrics.read_property_count(),
        read_property_multiple: metrics.read_property_multiple_count(),
        last_client,
        devices,
        log_lines,
    }
