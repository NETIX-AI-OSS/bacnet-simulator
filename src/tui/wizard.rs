...
pub fn draw(f: &mut Frame, area: ratatui::layout::Rect, wizard: &WizardState) {
    ...
    let fields = [
        (WizardField::BuildingName, format!("Building name: {}", wizard.building_name)),
        (
        ...
        (
            WizardField::Meters,
            format!("Meters: {}", if wizard.include_meters { "yes" } else { "no" }),
        ),
    ...
    let lines: Vec<Line> = fields
        .iter()
        .map(|(field, text)| {
            let marker = if wizard.selected == *field { "> " } else { "  " };
    ...
    f.render_widget(
        Paragraph::new(lines).block(Block::default().title(" Minimal config wizard ").borders(Borders::ALL)),
        chunks[0],
    );
