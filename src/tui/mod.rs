...
use crate::app::{AppContext, restart_process};
...
fn draw_ui(f: &mut ratatui::Frame, state: &TuiState) {
    ...
    let tabs = Tabs::new(titles)
        .block(Block::default().title(" BACnet Simulator ").borders(Borders::ALL))
        .select(selected)
        .style(Style::default())
    ...
    match state.tab {
        ...
        Tab::Devices => devices::draw(f, inner, &state.devices, &state.snapshot, &state.detail_points),
...
fn handle_key(
    ...
    match code {
        ...
        _ => match state.tab {
            ...
            Tab::Config => { handle_config_tab(state, code)?; }
            Tab::Status => {}
        },
    }
    Ok(())
...
fn handle_config_tab(state: &mut TuiState, code: KeyCode) -> Result<(), Box<dyn std::error::Error>> {
    match code {
        KeyCode::Up => {
